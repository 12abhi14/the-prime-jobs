...
def updateJob(request,pk):
    ...
    job.discription = request.data['discription']
...
def getTopicStats(request,topic):
    ...
    args = {'tittle__icontains':topic}
    jobs = Job.objects.filter(**args)

    if len(jobs) == 0:
        return Response({'message': 'No stats available for {topic}'.format(topic=topic)})
    
    stats = jobs.aaggregate(
